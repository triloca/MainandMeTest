...
NSString * const kkkkServerBaseURL = @"http://www.mainandme.com/api/v1";
...
- (void)deleteProduct:(NSString*)productId
    ...
    NSString* urlString =
    [NSString stringWithFormat:@"%@/products/%@?_token=%@", kkkkServerBaseURL, productId, [CommonManager shared].apiToken];
...
- (void)addStoreName:(NSString*)name toProductId:(NSString*)productId
             success:(void(^) (NSDictionary* prod)) success
              failure:(void(^) (NSError* error, NSString* errorString)) failure
            exception:(void(^) (NSString* exceptionString))exception{
    
    NSString* urlString =
    [NSString stringWithFormat:@"%@/products/%@?product[store_name]=%@&_token=%@", kkkkServerBaseURL, productId, name, [CommonManager shared].apiToken];
...
//
//    urlString = [NSString stringWithFormat:@"%@%@", kkServerBaseURL, urlString];
//    
...
- (void)uploadProductWithName:(NSString*)name
    ...
    NSString* urlString =
    [NSString stringWithFormat:@"%@/products", kkkkServerBaseURL];
    
    urlString = [NSString stringWithFormat:@"%@?_token=%@", urlString, [CommonManager shared].apiToken];
    ...
    //[parameters setValue:[CommonManager shared].apiToken forKey:@"token"];
    if (name.length > 0) {
    ...
    if (type.length > 0) {
        [parameters setValue:type forKey:@"product[state]"];
    }
...
- (void)uploadStoreWithName:(NSString*)name
    ...
    NSString* urlString =
    [NSString stringWithFormat:@"%@/stores", kkkkServerBaseURL];
...
- (void)uploadStoreWithName:(NSString*)name
    ...
    NSString* urlString =
    [NSString stringWithFormat:@"%@/products?_token=%@&product[name]=%@&product[price]=%@&product[store_name]=%@&product[category]=%@&product[description]=%@&image_name=image.jpg", kkkkServerBaseURL, [CommonManager shared].apiToken, name, price, storeName, category, description];
...
- (void)loadCommunitiesByLocation:(CLLocationCoordinate2D)coordinate
                    success:(void(^) (NSArray* objects)) success
                    failure:(void(^) (NSError* error, NSString* errorString)) failure
                  exception:(void(^) (NSString* exceptionString))exception{
    
    NSString* urlString =
    [NSString stringWithFormat:@"%@/communities?lat=%f&lng=%f", kkkkServerBaseURL, coordinate.latitude, coordinate.longitude];
